...
$sql = "SELECT * FROM TOURNOIS AS t LEFT JOIN PARTICIPATION AS p ON p.numero_tournois = t.id_tournois WHERE p.email = '$email_connecte' AND t.id_tournois IN (SELECT numero_tournois FROM PARTICIPATION WHERE email = '$email_connecte' AND participant = 1);";
