...
$sql = "SELECT t.*,
        ...
        WHERE t.id_tournois IN (
            ...
            FROM PARTICIPATION
            WHERE email='$email_connecte' AND participant=1)";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td class='buttons-column'>" . "<br>" . $row["nombre_participants"] . "/" . $row["place_maximum"] . "   participants<br>" . ($row["cash_prize"] > 0 ? "(" . $row["cash_prize"] . "€ à gagner) <br>" : "<br><br>") . "<br>";
        echo "<button onclick='showInfo(\"" . $row["plus_info"] . "\")'>+information</button></td>";
        
        echo "<td  class='buttons-column' style='height: 100%;'>";
        
        
        echo "<button type='button' style='height: 100%; width: 100%;' onclick='submitForm(\"quitter\", " . htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8') . ")' name='quitter'>QUITTER</button>";
        echo "</td>";
        echo "</tr>";
    }
    echo "</table>";
...
function submitForm(id_form, row) {
    ...
    form.setAttribute('id', 'quitter');
    form.setAttribute('method', 'POST');
    form.setAttribute('action', 'R4_quitter.php');
