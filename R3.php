...
<br>
<?php
function Demande_Accepter($conn, $email_connecte, $x) {
    // Requête SQL pour vérifier si la demande existe
    $sql = "SELECT COUNT(*) AS demande_count FROM DEMANDE WHERE demandeur = '$email_connecte' AND numero_tournois = $x AND accepter=1";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $demande_count = $row['demande_count'];
    
    // Retourner TRUE si la demande existe (le compteur est supérieur à 0), sinon retourner FALSE
    return ($demande_count > 0) ? true : false;
}
function Demande_Refuser($conn, $email_connecte, $x) {
    // Requête SQL pour vérifier si la demande existe
    $sql = "SELECT COUNT(*) AS demande_count FROM DEMANDE WHERE demandeur = '$email_connecte' AND numero_tournois = $x AND accepter=0";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $demande_count = $row['demande_count'];
...
$sql = "SELECT t.*,
        ...
        FROM TOURNOIS AS t
        JOIN DEMANDE AS d ON t.id_tournois = d.numero_tournois
        JOIN PARTICIPATION AS p ON t.id_tournois = p.numero_tournois
        WHERE t.id_tournois IN (
            SELECT numero_tournois
            FROM PARTICIPATION
            WHERE email='$email_connecte' AND demandeur=1
        )";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td><br> " . $row["date_tournois"] . " <br>" . $row["h_tournois"] . "H" . $row["m_tournois"] . "MIN</td>";
        
        echo "<td class='buttons-column'><br> " . ($row["prive"] == 1 ? "PRIVE" : "<div class='container'><p id='texteACopier'>" . $row['lieu_tournois'] . "</p><button onclick='copierLieu()'> Copier </button></div>") . "</td>" ;
        
        echo "<td class='buttons-column'><br> " . ($row["prive"] == 1 ? "PRIVE" :  "<div class='container'><p id='texteACopier2'>" . $row['numero_telephone'] . "</p><button onclick='copierNum()'> Copier </button></div>") . "</td>" ;
        
        echo "<td  class='buttons-column'>" . $row["nombre_participants"] . "/" . $row["place_maximum"] . "   participants<br><br>" . ($row["cash_prize"] > 0 ? "(" . $row["cash_prize"] . "€ à gagner) <br>" : "<br>") . "<br>";
        echo "<button onclick='showInfo(\"" . $row["plus_info"] . "\")'>+information</button></td>";
        
        echo "<td  class='buttons-column'>";
        
        if (Demande_Accepter($conn, $email_connecte, $row["id_tournois"])){
            echo "Accepter<br>";
            }
        else if (Demande_Refuser($conn, $email_connecte, $row["id_tournois"])){
            echo "refuser<br>";
            }
        else{
            echo "en attente<br>";
            echo '<button type="button" onclick="submitMessForm(' . htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8') . ')" style="height: 8vh;">Messagerie</button>';

            echo "<button type='button' onclick='submitForm(\"annuler_demande\", " . htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8') . ")' style='height: 8vh;'> ANNULER </button>";
...
function submitForm(id_form, row) {
    ...
    form.setAttribute('id', 'annuler_demande');
    form.setAttribute('method', 'POST');
    form.setAttribute('action', 'R3_annuler.php');
    ...
    inputEmailConnecte.setAttribute('value', '<?php echo $email_connecte; ?>'); // Assurez-vous que $email_connecte est disponible ici
...
function submitMessForm(row) {
    ...
    inputPretendant.setAttribute('value', row['createur']);
